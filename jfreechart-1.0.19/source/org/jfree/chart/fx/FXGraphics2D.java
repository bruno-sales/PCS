...
public class FXGraphics2D extends Graphics2D {
    ...
    /**
     * The width of the stroke to use when the user supplies a BasicStroke with
     * a width of 0.0 (in this case the Java specification says "If width is set
     * to 0.0f, the stroke is rendered as the thinnest possible line for the
    ...
    /**
     * An instance that is lazily instantiated in drawLine and then subsequently
     * reused to avoid creating a lot of garbage.
     */
    private Line2D line;

    /**
     * An instance that is lazily instantiated in fillRect and then subsequently
     * reused to avoid creating a lot of garbage.
     */
    Rectangle2D rect;
    ...
    /**
     * Returns the width to use for the stroke when the AWT stroke specified has
     * a zero width (the default value is {@code 0.5}).
     * <p>
     * In the Java specification for {@code BasicStroke} it states "If width is
     * set to 0.0f, the stroke is rendered as the thinnest possible line for the
     * target device and the antialias hint setting." We don't have a means to
     * implement that accurately since we must specify a fixed width to the
     * JavaFX canvas - this attribute is the width that is used.</p>
     *
     * @return The width.
     */
    ...
    /**
     * Sets the width to use for the stroke when the current AWT stroke has a
     * width of {@code 0.0}.
     *
     ...
     * {@code false} (the clipping is ENABLED) but since it does not always work
     * correctly you have the option to disable it. See
    ...
    /**
     * Sets the paint used to draw or fill shapes (or text). If {@code paint} is
     * an instance of {@code Color}, this method will also update the current
     * color attribute (see {@link #getColor()}). If you pass {@code null} to
     * this method, it does nothing (in accordance with the JDK specification).
     * <br><br>
     * Note that this implementation will map {@link Color},
     * {@link GradientPaint}, {@link LinearGradientPaint} and
     * {@link RadialGradientPaint}, other paint implementations are not handled.
     *
    ...
    /**
     * Sets the foreground color. This method exists for backwards compatibility
     * in AWT, you should use the {@link #setPaint(java.awt.Paint)} method.
     *
     ...
     * that too (but for that case, the {@link #clearRect(int, int, int, int)}
    ...
    /**
     * Maps a line join code from AWT to the corresponding JavaFX StrokeLineJoin
     * enum value.
     *
    ...
    /**
     * Returns the current value for the specified hint. Note that all hints are
     * currently ignored in this implementation.
     *
     * @param hintKey the hint key ({@code null} permitted, but the result will
     * be {@code null} also in that case).
     *
    ...
    /**
     * Sets the value for a hint. Note that all hints are currently ignored in
     * this implementation.
     *
    ...
    /**
     * Returns a copy of the rendering hints. Modifying the returned copy will
     * have no impact on the state of this {@code Graphics2D} instance.
     *
     ...
     * {@code Rectangle2D}, {@code Ellipse2D}, {@code Arc2D} and {@code Path2D}.
    ...
    /**
     * Fills the specified shape with the current {@code paint}. There is direct
     * handling for {@code RoundRectangle2D}, {@code Rectangle2D},
     * {@code Ellipse2D} and {@code Arc2D}. All other shapes are mapped to a
     * path outline and then filled.
     *
     * @param s the shape ({@code null} not permitted).
     *
     ...
     * {@code FontRenderContext} for an image that is maintained internally (as
     * for {@link #getFontMetrics}).
     *
    ...
    /**
     * Draws a string at {@code (x, y)}. The start of the text at the baseline
     * level will be aligned with the {@code (x, y)} point.
     *
    ...
    /**
     * Applies the translation {@code (tx, ty)}. This call is delegated to
    ...
    /**
     * Applies a shear transformation. This is equivalent to the following call
     * to the {@code transform} method:
     * <br><br>
     * <ul><li> {@code transform(AffineTransform.getShearInstance(shx, shy));}
     * </ul>
     *
    ...
    /**
     * Returns {@code true} if the rectangle (in device space) intersects with
     * the shape (the interior, if {@code onStroke} is false, otherwise the
     ...
     * {@code null} argument, but there is an open bug report (since 2004) that
    ...
    /**
     * Draws a line from {@code (x1, y1)} to {@code (x2, y2)} using the current
    ...
    /**
     * Clears the specified rectangle by filling it with the current background
     * color. If the background color is {@code null}, this method will do
     * nothing.
     *
     * @param x the x-coordinate.
     * @param y the y-coordinate.
    ...
    /**
     * Draws a rectangle with rounded corners using the current {@code paint}
     * and {@code stroke}.
     *
     * @param x the x-coordinate.
     * @param y the y-coordinate.
    ...
    /**
     * Draws an oval framed by the rectangle {@code (x, y, width, height)} using
     ...
     * {@code (x, y, width, height)}, starting at {@code startAngle} and
     * continuing through {@code arcAngle} degrees using the current
     * {@code paint} and {@code stroke}.
     *
     * @param x the x-coordinate.
     * @param y the y-coordinate.
    ...
    /**
     * Draws the specified multi-segment line using the current {@code paint}
     * and {@code stroke}.
     *
     ...
     * Draws part of an image (defined by the source rectangle
     * {@code (sx1, sy1, sx2, sy2)}) into the destination rectangle
     * {@code (dx1, dy1, dx2, dy2)}. The destination rectangle is first cleared
     * by filling it with the specified {@code bgcolor}. Note that the
     * {@code observer} is ignored.
     *
     * @param img the image.
     * @param dx1 the x-coordinate for the top left of the destination.
    ...
    /**
     * Converts a rendered image to a {@code BufferedImage}. This utility method
    ...
    /**
     * Draws the image resulting from applying the {@code BufferedImageOp} to
     * the specified image at the location {@code (x, y)}.
     *
     * @param img the image.
    ...
    /**
     * Sets the attributes of the reusable {@link Ellipse2D} object that is used
     * by the {@link #drawOval(int, int, int, int)} and
