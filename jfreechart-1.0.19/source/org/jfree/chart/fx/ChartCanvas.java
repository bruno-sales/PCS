 ...
 * that we can incorporate feedback on the (new) JavaFX support in
 * JFreeChart.</p>
 *
 * @since 1.0.18
 */
public class ChartCanvas extends Canvas implements ChartChangeListener {
     ...
     * default value is {@code true}. The {@link TooltipHandlerFX} class will
     * only update the tooltip if this flag is set to {@code true}.
     *
     * @return The flag.
     */
     ...
     * is used to determine the position of the crosshairs on the chart, if they
     * are visible).
     *
    ...
    /**
     * Returns the mouse handler with the specified ID, or {@code null} if there
     * is no handler with that ID. This method will look for handlers in both
     * the regular and auxiliary handler lists.
     *
     ...
     * candidates to take the position of live handler). The handler must have
     * an ID that uniquely identifies it amongst the handlers registered with
     * this canvas.
     *
     ...
     * anchor. If the text is {@code null}, no tooltip will be displayed. This
     * method is intended for calling by the {@link TooltipHandlerFX} class, you
     * won't normally call it directly.
     *
    ...
    /**
     * Handles a mouse pressed event by (1) selecting a live handler if one is
     * not already selected, (2) passing the event to the live handler if there
     * is one, and (3) passing the event to all enabled auxiliary handlers.
     *
