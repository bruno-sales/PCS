...
public class LogarithmicAxis extends NumberAxis {
    ...
    /**
     * Sets the 'allowNegativesFlag' flag; true to allow negative values in
     * data, false to be able to plot positive values arbitrarily close to zero.
     *
    ...
    /**
     * Returns the 'allowNegativesFlag' flag; true to allow negative values in
     * data, false to be able to plot positive values arbitrarily close to zero.
     *
     * @return The flag.
     */
    ...
    /**
     * Sets the 'expTickLabelsFlag' flag. If the 'log10TickLabelsFlag' is false
     * then this will set whether or not "1e#"-style tick labels are used. The
     * default is to use regular numeric tick labels.
     *
     * @param flgVal true for "1e#"-style tick labels, false for log10 or
     ...
     *
     * @return <code>true</code> for "1e#"-style tick labels, <code>false</code>
     ...
     *
     * @param flag true for "10^n"-style tick labels, false for "1e#"-style or
     * regular numeric tick labels.
     */
     ...
     * <code>false</code> for "1e#"-style or regular numeric tick labels.
     */
     ...
     *
     * @param flag <code>true</code> to make the 'autoAdjustRange()' method
     * select the next "10^n" values, <code>false</code> to not.
     */
     ...
     *
     * @return <code>true</code> if the 'autoAdjustRange()' method will select
     * the next "10^n" values, <code>false</code> if not.
     */
    ...
    /**
     * Sets up flag for log axis processing. Set true if negative values not
     * allowed and the lower bound is between 0 and 10.
     */
     ...
     * plotted. If negative values are not allowed and the lower bound is
     * between 0 and 10 then a normal log is returned; otherwise the returned
     * value is adjusted if the given value is less than 10.
     *
     * @param val the value.
     *
    ...
    /**
     * Returns a power of 10, depending on if values between 0 and 1 are being
     * plotted. If negative values are not allowed and the lower bound is
     * between 0 and 10 then a normal power is returned; otherwise the returned
     * value is adjusted if the given value is less than 1.
     *
     * @param val the value.
     *
     ...
     * adjustment is that negative values are changed to positive during the
     * calculations, and then the answer is negated at the end. The second is
     * that, for values less than 10, an increasingly large (0 to 1) scaling
     * factor is added such that at 0 the value is adjusted to 1, resulting in a
     * returned result of 0.
     *
     ...
     * adjustment is that negative values are changed to positive during the
     * calculations, and then the answer is negated at the end. The second is
     * that, for values less than 1, a progressive logarithmic offset is
     * subtracted such that at 0 the returned result is also 0.
     *
