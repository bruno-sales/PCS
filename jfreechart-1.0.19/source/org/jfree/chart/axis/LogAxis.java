...
/**
 * A numerical axis that uses a logarithmic scale. The class is an alternative
 * to the {@link LogarithmicAxis} class.
 *
 * @since 1.0.7
 */
public class LogAxis extends ValueAxis {
    ...
    /**
     * The base symbol to display (if {@code null} then the numerical value of
     * the base is displayed).
     */
    private String baseSymbol = null;

    /**
     * The formatter to use for the base value when the base is displayed as a
     * numerical value.
     */
     ...
     * for the axis. If this is {@code null} (the default) then the numerical
    ...
    /**
     * Sets the symbol used to represent the base value of the logarithmic scale
    ...
    /**
     * Sets the formatter used to format the base value of the logarithmic scale
     * when it is displayed numerically and sends a change event to all
    ...
    /**
     * Returns the number format override. If this is non-{@code null}, then it
     * will be used to format the numbers on the axis.
     *
     ...
     * registered listeners. If this is non-{@code null}, then it will be used
     * to format the numbers on the axis.
     *
    ...
    /**
     * Converts a Java2D coordinate to an axis value, assuming that the axis is
     * aligned to the specified {@code edge} of the {@code area}.
     *
     * @param java2DValue the Java2D coordinate.
     * @param area the area for plotting data ({@code null} not permitted).
    ...
    /**
     * Converts a value on the axis scale to a Java2D coordinate relative to the
     * given {@code area}, based on the axis running along the specified
     * {@code edge}.
     *
    ...
    /**
     * Configures the axis. This method is typically called when an axis is
     * assigned to a new plot.
     */
    @Override
    ...
    /**
     * Creates a tick label for the specified value based on the current tick
     * unit (used for formatting the exponent).
     *
     * @param value the value.
     *
     * @return The label.
    ...
    /**
     * Resizes the axis length to the specified percentage of the current range
     * and sends a change event to all registered listeners. If {@code percent}
     * is greater than 1.0 (100 percent) then the axis range is increased (which
     * has the effect of zooming out), while if the {@code percent} is less than
     * 1.0 the axis range is decreased (which has the effect of zooming in). The
     * resize occurs around an anchor value (which may not be in the center of
     * the axis). This is used to support mouse wheel zooming around an
     * arbitrary point on the plot.
     * <br><br>
    ...
    /**
     * Returns a collection of tick units for log (base 10) values. Uses a given
     ...
     * from a future version. If you need this method, copy the source code into
     * your project.
     */
