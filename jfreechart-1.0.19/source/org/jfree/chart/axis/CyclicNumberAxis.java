 ...
 * </pre>
 *
 * Here, the range bounds are at the axis extremities. With cyclic axis,
 * however, the time is split in "cycles", or "time frames", or the same
 * duration : the period.
 *
 * A cycle axis cannot by definition handle a larger interval than the period :
 * <pre>x1 - x0 &gt;= period</pre>. Thus, at most a full period can be
 * represented with such an axis.
 *
 * The cycle bound is the number between x0 and x1 which marks the beginning of
 * new time frame:
 * <pre>
 * |---------------------|----------------------------|
 * x0                   cb                           x1
 ...
 *
 * Thus, by definition, two consecutive cycle bounds period apart, which is
 * precisely why it is called a period.
 *
 ...
 * </pre>
 *
 * The cycle bound is at the axis ends, then current cycle is shown, then the
 * last cycle. When using dynamic data, the visual effect is the current cycle
 * erases the last cycle as x grows. Then, the next cycle bound is reached, and
 * the process starts over, erasing the previous cycle.
 *
...
public class CyclicNumberAxis extends NumberAxis {
     ...
     * last cycle. It's up to the user's choice to decide which, as this is just
     * a convention. By default, the cycle bound is mapped to the current cycle.
     * <br>
     ...
     *
     * @return <code>true</code> if the cycle bound is mapped to the last cycle,
     * <code>false</code> if it is bound to the current cycle (default)
     */
    ...
    /**
     * Builds a list of ticks for the axis. This method is called when the axis
    ...
    /**
     * Builds a list of ticks for the axis. This method is called when the axis
     * is at the left or right of the chart (so the axis is "vertical").
     *
     * @param g2 the graphics device.
     * @param dataArea the data area.
     * @param edge the edge.
    ...
    /**
     * The auto range is fixed for this class to the period by default. This
     * function will thus set a new period.
     *
    ...
    /**
     * The cycle bound is defined as the higest value x such that "offset +
     * period * i = x", with i and integer and x &lt; range.getUpperBound() This
     * is the value which is at both ends of the axis : x...up|low...x The
     * values from x to up are the valued in the current cycle. The values from
     * low to x are the valued in the previous cycle.
     *
