 ...
 * supports. This class implements a timeline formed by segments of equal length
 * (ex. days, hours, minutes) where some segments can be included in the
 ...
 * Examples of segments are: days, hours, minutes, etc. The size of a segment is
 * defined as the number of milliseconds in the segment. Some useful segment
 ...
 * <p>
 * For example, a working days SegmentedTimeline would be formed by a group of 7
 * daily segments, where there are 5 included (Monday through Friday) and 2
 ...
 * <p>
 * In the example, the following segment attributes are presented:</p>
 * <ul>
 ...
 * <p>
 * Additionally the {@code startTime}, or start of the first Segment of the
 * smallest segment group needs to be defined. This startTime could be relative
 * to January 1, 1970, 00:00:00 GMT or any other date. This creates a point of
 * reference to start counting Segment Groups. For example, for the working days
 * SegmentedTimeline, the {@code startTime} could be 00:00:00 GMT of the first
 * Monday after January 1, 1970. In this class, the constant
 * FIRST_MONDAY_AFTER_1900 refers to a reference point of the first Monday of
 * the last century.</p>
 * <p>
 * A SegmentedTimeline can include a baseTimeline. This combination of timelines
 * allows the creation of more complex timelines. For example, in order to
 * implement a SegmentedTimeline for an intraday stock trading application,
 * where the trading period is defined as 9:00 AM through 4:00 PM Monday through
 * Friday, two SegmentedTimelines are used. The first one (the baseTimeline)
 * would be a working day SegmentedTimeline (daily timeline Monday through
 * Friday). On top of this baseTimeline, a second one is defined that maps the
 * 9:00 AM to 4:00 PM period. Because the baseTimeline defines a timeline of
 * Monday through Friday, the resulting (combined) timeline will expose the
 * period 9:00 AM through 4:00 PM only on Monday through Friday, and will remove
 * all other intermediate intervals.</p>
 * <p>
...
public class SegmentedTimeline implements Timeline, Cloneable, Serializable {
     ...
     * constant.
     *
     * @deprecated As of 1.0.7. This field doesn't take into account changes to
     ...
     *
     * @deprecated As of 1.0.7. This field is initialised based on the default
     * time zone, and doesn't take into account subsequent changes to the
     * default.
     */
     ...
     * calculations.
     *
     * @deprecated As of 1.0.7. When the default time zone is required, just
     * call {@code TimeZone.getDefault()}.
     */
    ...
    /**
     * Start of time reference from time zero (1/1/1970). This is the start of
     * segment #0.
     */
     ...
     * <P>
     * The {@code startTime} of the resulting timeline will be midnight of the
     * first Monday after 1/1/1900.
     *
     ...
     * <P>
     * This timeline uses a segmentSize of FIFTEEN_MIN_SEGMENT_SIZE. The segment
     * group is defined as 28 included segments (9:00 AM through 4:00 PM) and 68
     * excluded segments (4:00 PM through 9:00 AM the next day).
     * <P>
     * In order to exclude Saturdays and Sundays it uses a baseTimeline that
     * only includes Monday through Friday days.
     * <P>
     * The {@code startTime} of the resulting timeline will be 9:00 AM after the
     * startTime of the baseTimeline. This will correspond to 9:00 AM of the
     * first Monday after 1/1/1900.
     *
     * @return A fully initialized SegmentedTimeline.
     */
    ...
    /**
     * Returns the start time for the timeline. This is the beginning of segment
     * zero.
     *
...
//                }
    ...
    /**
     * Returns {@code true} if a range of values are contained in the timeline.
     * This is implemented verifying that all segments are in the range.
     *
     ...
     * segment of the timeline. An exception segment can not be contained inside
     * an already excluded segment. If so, no action will occur (the proposed
     ...
     * segment and ends with a domainValue that ends a valid segment. Therefore
     * the range will contain all segments whose segmentStart &lt;= domainValue
     * and segmentEnd &lt;= toDomainValue.
     *
     ...
     * defined as a segment to exclude from what would otherwise be considered a
     * valid segment of the timeline. An exception segment can not be contained
     ...
     * segment to exclude from what would otherwise be considered a valid
     * segment of the timeline. An exception segment can not be contained inside
     * an already excluded segment. This is verified inside this method, and if
     * so, no action will occur (the proposed exception segment will be
     * discarded).
     *
     ...
     * <p>
     * An exception segment is defined as a segment to exclude from what would
    ...
    /**
     * Adds a segment relative to the baseTimeline as an exception. An exception
     * segment is defined as a segment to exclude from what would otherwise be
     * considered a valid segment of the timeline. An exception segment can not
     * be contained inside an already excluded segment. If so, no action will
     * occure (the proposed exception segment will be discarded).
     * <p>
     * The segment is identified by a domainValue into any part of the segment.
     * Therefore the segmentStart &lt;= domainValue &lt;= segmentEnd.
     *
    ...
    /**
     * Returns a segment that contains a domainValue. If the domainValue is not
     * contained in the timeline (because it is not contained in the
     * baseTimeline), a Segment that contains {@code index + segmentSize*m} will
     * be returned for the smallest {@code m} possible.
     *
    ...
    /**
     * Returns a segment that contains a date. For accurate calculations, the
     * calendar should use TIME_ZONE for its calculation (or any other similar
     * time zone).
     *
     * If the date is not contained in the timeline (because it is not contained
     * in the baseTimeline), a Segment that contains
     * {@code date + segmentSize*m} will be returned for the smallest {@code m}
     * possible.
     *
     ...
     *
     * @return {@code true} if both objects are equal or both {@code null},
     ...
     * <i>insertion point</i> is defined as the point at which the segment would
     * be inserted into the list: the index of the first element greater than
     * the key, or <tt>list.size()</tt>, if all elements in the list are less
     * than the specified segment. Note that this guarantees that the return
    ...
    public class Segment implements Comparable, Cloneable, Serializable {
         ...
         *
         * @return {@code true} if the millisecond is contained in the segment.
         */
        ...
        /**
         * Returns {@code true} if an interval is contained in this segment.
         *
         * @param from the start of the interval.
         * @param to the end of the interval.
         *
         * @return {@code true} if the interval is contained in the segment.
         */
         ...
         *
         * @return {@code true} if the segment is contained in this segment.
         */
        ...
        /**
         * Returns {@code true} if this segment is wholly after another segment.
         *
         * @param other the other segment.
         *
         * @return A boolean.
        ...
        /**
         * Tests an object (usually another {@code Segment}) for equality with
         * this segment.
         *
        ...
        /**
         * Returns a copy of ourselves or {@code null} if there was an exception
         * during cloning.
         *
         ...
         *
         * @return -1: this &lt; object, 0: this.equal(object) and +1: this &gt;
         * object
         */
        @Override
        ...
        /**
         * Calculate the segment number relative to the segment group. This will
         * be a number between 0 and segmentsGroup-1. This value is calculated
         * from the segmentNumber. Special care is taken for negative
         * segmentNumbers.
         *
         * @return The segment number.
         */
