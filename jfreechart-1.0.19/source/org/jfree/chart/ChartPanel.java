...
public class ChartPanel extends JPanel implements ChartChangeListener,
     ...
     *
     * @param flag  <code>true</code> enables trace lines for the mouse pointer
     ...
     * the user can specify the filename). Note that this method only works if
     * the JFreeSVG library is on the classpath...if this library is not
     ...
     * This feature is only supported if the JFreeSVG library is included on the
     * classpath.
     *
     * @return A string containing an SVG element for the current chart, or
     * <code>null</code> if there is a problem with the method invocation by
     * reflection.
     */
     ...
     * the user can specify the filename). Note that this method only works if
     * the OrsonPDF library is on the classpath...if this library is not
     * present, the method will fail.
     */
    ...
    /**
     * Writes the current chart to the specified file in PDF format. This will
     * only work when the OrsonPDF library is found on the classpath. Reflection
     * is used to ensure there is no compile-time dependency on OrsonPDF (which
     * is non-free software).
     *
