 ...
 * <code>PaintAlpha</code> was originally conceived to improve the rendering of
 * 3D Shapes with transparent colours and to allow invisible bars by making them
 * completely transparent.
 * <p>
 * In, for example
 * {@link org.jfree.chart.renderer.category.StackedBarRenderer3D StackedBarRenderer3D},
 * bars are rendered with 6 faces. The front face is rendered with the
 * <code>Paint</code> requested. The other 5 faces are rendered darker to
 * achieve the 3D effect.
 * <p>
 * Previously {@link Color#darker()} was used for this, which always returns an
 * opaque colour.
 * <p>
 * Additionally there are methods to control the behaviour and in particular a
 * {@link PaintAlpha#cloneImage(BufferedImage) cloneImage(..)} method which is
 * needed to darken objects of type {@link TexturePaint}.
 *
 * @author DaveLaw
 *
 * @since 1.0.15
...
public class PaintAlpha {
     ...
     * information. In the past this was not the case. If you wish legacy
     * functionality for your application you can request this here.
     *
     ...
     * If the Type is not supported, the original <code>Paint</code> is
     * returned.
     * <p>
     * @param paint a <code>Paint</code> implementation (e.g.
     * {@link Color}, {@link GradientPaint}, {@link TexturePaint},..)
     * <p>
     ...
     * <p>
     * The essential difference is that this method maintains the alpha-channel
     * unchanged<br>
     *
     ...
     * <p>
     * This entails cloning the underlying <code>BufferedImage</code>, then
     * darkening each color-pixel individually!
     *
    ...
    private static TexturePaint darkerTexturePaint(TexturePaint paint) {
        /**
         * Color Models with pre-multiplied Alpha tested OK without any special
         * logic
         *
        ...
        /**
         * Indexed Color Models (sort of a Palette) CANNOT be simply multiplied
         * (the pixel-value is just an index into the Palette).
         *
         ...
         *
         * It is quite possible that the "nearest" color is the ORIGINAL color!
         * In the worst case, the returned Image will be identical to the
         * original.
         *
        ...
        if (img.getSampleModel().getNumBands() == 4) {
             ...
             * BufferedImage.TYPE_USHORT_565_RGB: // Type 08: tested OK
             * 2011.02.27 BufferedImage.TYPE_USHORT_555_RGB: // Type 09: tested
             * OK 2011.02.27
             *
             * Note: as ras.getPixels(..) returned colours in the order R, G, B,
             * A (optional) for both TYPE_4BYTE_ABGR & TYPE_3BYTE_BGR, it is
             * assumed that TYPE_INT_BGR will behave similarly.
             */
        }
    }
     ...
     * known Color Models<br>      ({@link java.awt.image.IndexColorModel     IndexColorModel},
