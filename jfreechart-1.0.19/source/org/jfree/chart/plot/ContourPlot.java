...
public class ContourPlot extends Plot implements ContourValuePlot,
     ...
     * width (of the dataArea). Modifies plot area calculations. ratio &gt; 0
     * will attempt to layout the plot so that the
     * dataArea.height/dataArea.width = ratio. ratio &lt; 0 will attempt to
     * layout the plot so that the dataArea.height/dataArea.width in plot units
     * (not java2D units as when ratio &gt; 0) = -1.*ratio.
     */         //dmo
     ...
     * rendering of the plot (dimensions, tooltip information etc). Just pass in
...
//      rect = new Rectangle2D.Double();
    ...
    /**
     * Multiplies the range on the domain axis/axes by the specified factor (to
     * be implemented).
     *
