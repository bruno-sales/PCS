 ...
 * "spider web". Multiple series can be plotted on the same axis to allow easy
 * comparison. This plot doesn't support negative values at present.
 */
public class SpiderWebPlot extends Plot implements Cloneable, Serializable {
    ...
    /**
     * The default maximum value plotted - forces the plot to evaluate the
     * maximum from the data passed in
     */
    ...
    /**
     * The data extract order (BY_ROW or BY_COLUMN). This denotes whether the
     * data series are stored in rows (in which case the category names are
    ...
    /**
     * Returns the direction in which the radar axes are drawn (clockwise or
     * anti-clockwise).
     *
    ...
    /**
     * Sets the URL generator for the plot and sends a {@link PlotChangeEvent}
     * to all registered listeners.
     *
     * @param generator the generator (<code>null</code> permitted).
     *
    ...
    /**
     * loop through each of the series to get the maximum value on each category
     * axis
     *
    ...
    /**
     * Returns the value to be plotted at the interseries of the series and the
     * category. This allows us to plot <code>BY_ROW</code> or
     * <code>BY_COLUMN</code> which basically is just reversing the definition
     * of the categories and data series being plotted.
     *
     ...
     *
     * @throws CloneNotSupportedException if the plot cannot be cloned for any
     * reason.
     */
    @Override
